...
def advance_along_forward(
    *,
    ...
    print(
        f"[{label}] 沿前进方向补偿 {distance * 1000:.0f} mm "
    ...
    *,
    ...
    for it in range(max(1, int(max_iters))):
        ...
        for j, arm_ticks in enumerate(route_ticks):
            pose = full_pose_from_ik(arm_ticks, g_open, ticks)
            go_pose_if_changed(arm, f"preclose{it+1}_wp{j+1}", pose)
            time.sleep(0.01)
...
def go_pose_if_changed(arm: TicksArm, name: str, pose: dict, min_delta_ticks: float = 3.0) -> bool:
    """Skip route points that quantize to the same servo ticks."""
    if arm.goal:
        delta = max(
            abs(float(pose.get(j, arm.goal.get(j, 0.0))) - float(arm.goal.get(j, 0.0)))
            for j in pose
        )
        if delta < float(min_delta_ticks):
            print(f"[skip] {name}: ticks unchanged")
            return False
...
def main() -> int:
    ...
    parser.add_argument(
        "--retry-forward-compensation-m",
        type=float,
        default=0.0,
        help="空握后、重新视觉规划前沿本次前进方向额外补偿的距离(m)",
    )
    parser.add_argument(
        "--retry-forward-route-points",
        type=int,
        default=2,
        help="空握补偿的水平路点数",
    )
    parser.add_argument(
        "--final-forward-probe-m",
        type=float,
        default=0.0,
        help="最终闭爪前沿抓取前进方向额外探入的距离(m)",
    )
    parser.add_argument(
        "--final-forward-route-points",
        type=int,
        default=2,
        help="最终闭爪前探的水平路点数",
    )
    parser.add_argument("--gripper-open", type=float, default=None)
    parser.add_argument("--open-gripper-settle-s", type=float, default=0.4)
    parser.add_argument(
        "--gripper-close",
        type=float,
        default=800.0,
    ...
    parser.add_argument(
        "--preclose-fail-forward",
        type=float,
        default=0.025,
        help="闭爪前对准失败时先水平前探的距离(m)",
    )
    parser.add_argument(
    ...
    parser.add_argument("--retry-open-settle-s", type=float, default=0.45)
    parser.add_argument("--preclose-settle-s", type=float, default=0.35)
    parser.add_argument("--close-settle-s", type=float, default=0.85)
    parser.add_argument(
        "--grasp-retries",
        type=int,
        default=3,
    ...
    parser.add_argument(
        "--return-to-observe",
        action="store_true",
        help="确认夹住后，夹爪保持实际位置，仅将机械臂回到观察位并退出",
    )
    ...
    try:
        sequence_cfg = cfg.get("sequence") or {}
        ...
        for connect_attempt in range(1, max_connect_attempts + 1):
            try:
                ...
                if connect_attempt >= max_connect_attempts:
                    print(
                        f"[connect] {max_connect_attempts} 次连接均失败，停止重试"
                    )
                    raise
                print(
                    f"[connect] 第 {connect_attempt}/{max_connect_attempts} 次握手失败: "
                    f"{exc}; {connect_retry_s:.1f}s 后重试"
                )
        ...
        if args.open_gripper:
            set_gripper(arm, g_open, limits, float(args.open_gripper_settle_s))
            ticks_now = arm.read()
        ...
        for attempt in range(1, max_tries + 1):
            ...
            set_gripper(arm, g_open, limits, float(args.retry_open_settle_s))
            ...
            forward_xy = target_grasp[:2] - cur_xyz[:2]
            print(
            ...
            for i, arm_ticks in enumerate(route_ticks):
                ...
                go_pose_if_changed(arm, f"a{attempt}_wp{i+1}", pose)
                time.sleep(0.01)
            ...
            if do_preclose:
                ...
                graspable = refine_pose_until_graspable(
                    ...
                    settle_s=float(args.preclose_settle_s),
                )
                if not graspable and not args.close_even_if_ungraspable:
                    print(
                        f"[grasp] 未对准，先水平前探 {args.preclose_fail_forward * 1000:.0f} mm，"
                        "再重新视觉规划"
                    )
                    try:
                        advance_along_forward(
                            ...
                            distance_m=float(args.preclose_fail_forward),
                            route_points=2,
                            ik_tol=float(args.ik_tol),
                            max_horiz_m=float(args.max_horiz_m),
                            label=f"a{attempt}_preclose_forward",
                        )
                    except RuntimeError as exc:
                        print(f"[warn] 闭爪前水平前探失败: {exc}")
                    if attempt < max_tries:
                        continue
                    print("[grasp] 最后一次尝试：忽略视觉对准结果，强制闭爪")
                    graspable = True
                if not graspable:
            ...
            try:
                advance_along_forward(
                    ...
                    observe_arm=observe_arm,
                    forward_xy=forward_xy,
                    distance_m=float(args.final_forward_probe_m),
                    route_points=int(args.final_forward_route_points),
                    ik_tol=float(args.ik_tol),
                    max_horiz_m=float(args.max_horiz_m),
                    label=f"a{attempt}_final_probe",
                )
            except RuntimeError as exc:
                print(f"[warn] 最终前探失败，按当前位置闭爪: {exc}")

            print(f"[grasp] 闭爪 -> {int(g_close)}")
            set_gripper(arm, g_close, limits, float(args.close_settle_s))
            actual = arm.read()
            ...
            if gripper_closed_empty(actual_g, g_close, miss_tol):
                ...
                try:
                    advance_along_forward(
                        ...
                        observe_arm=observe_arm,
                        forward_xy=forward_xy,
                        distance_m=float(args.retry_forward_compensation_m),
                        route_points=int(args.retry_forward_route_points),
                        ik_tol=float(args.ik_tol),
                        max_horiz_m=float(args.max_horiz_m),
                        label=f"a{attempt}_retry_compensation",
                    )
                except RuntimeError as exc:
                    print(f"[warn] 空握前向补偿失败，直接重新规划: {exc}")
                continue
        ...
        if args.return_to_observe:
            # 不再重发 g_close，防止夹住物体后持续硬顶而触发舵机过载保护。
            actual = arm.read()
            return_pose = {
                joint: float(observe_pose.get(joint, actual.get(joint, 2048)))
                for joint in JOINTS
            }
            return_pose["gripper"] = float(actual.get("gripper", g_close))
            print(
                "[grasp] 已夹住：保持 gripper="
                f"{return_pose['gripper']:.0f}，回到观察位后退出"
            )
            go_pose_strict(arm, "grasped_return_observe", return_pose)
            if not args.no_save_image:
                # 回到观察位后只采图，不再发送任何机械臂或夹爪指令。
                time.sleep(0.2)
                after_frame = capture_frame(camera_cfg)
                after_det = detector.detect(after_frame)
                save_detection_image(
                    save_dir,
                    after_frame,
                    detector,
                    after_det,
                    tag="after_return_observe",
                )
            return 0
