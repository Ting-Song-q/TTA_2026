...
def build_motors(names: Optional[List[str]] = None):
    try:
        from lerobot.motors import Motor, MotorNormMode
    except ImportError:
        from lerobot.motors.motors_bus import Motor, MotorNormMode

    selected = list(MOTORS.keys()) if names is None else list(names)
    motors = {}
    for name in selected:
        if name not in MOTORS:
            raise KeyError("未知舵机名: %s" % name)
        cfg = MOTORS[name]
...
def connect_bus(
    ...
    motor_names: 仅握手这些舵机（例如缺夹爪时传不含 gripper 的列表）。
    """
    from lerobot.motors.feetech import FeetechMotorsBus

    rate = DEFAULT_BAUD if baud is None else baud
    motors = build_motors(motor_names)
    ...
    try:
        ...
        pass
    print(
        f"[OK] {port} @ baud={rate} protocol={PROTOCOL_VERSION} "
        f"motors={list(motors.keys())}"
    )
    try:
        bus._handshake()
    except Exception:
