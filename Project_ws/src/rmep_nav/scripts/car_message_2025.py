...
class Nav:
    ...
    def nav_for_projecte(self):
        print("nav start")
        self.download_picture(remote_path="/root/catkin_ws/pictures/1440x1088_1-1.jpg",
                              local_path="/home/tta/Project_ws/src/rmep_nav/scripts/test_message_2025/p1.jpg")
        print("stfp is over")
        
        #小车运动
        rospy.sleep(3)
        print("now I want to test 1")

        # 现在车运动到达第一个区域，向飞机发出信号
        self.please_takeoff('/mnt/takeoff.txt')
        print("the car has arrived the first goal and the plane can take off!!!!!!!!!!!!!!")
        
        rospy.sleep(3)
        print("now I want to test 2")

        #飞机在货架一识别结束，小车和飞机分别进入区域二
        self.is_go_2('/mnt/go_2.txt')

        rospy.sleep(3)
        print("now I want to test 3")

        #飞机降落完成，小车和飞机准备一起进入充电区
        self.is_go_charge('/mnt/go_charge.txt')
        print("go to charge region!!!!!!!!!!!!!!")
    ...
    def download_picture(self,remote_path,local_path):
