...
def main() -> int:
    ...
    try:
        ...
        if approach_status == "no_tag":
            # exit 2：供导航前移重试；不 hold_forever，避免卡住任务子进程
            print("[place] 粗接近失败：未检测到 AprilTag (exit=2)")
            return 2
        ...
        if not initial:
            ...
            home = {
                n: float(initial.get(n, observe_arm.get(n, 2048)))
                for n in JOINT_ORDER
            }
            # 已开爪则保持开；--no-open 时仍用当前持块爪值
            if args.no_open:
                try:
