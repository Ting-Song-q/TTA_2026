...
class TimedSpeedNav(object):
    ...
    def go_lateral_y(self, distance, speed=None):
        ...
        rospy.loginfo(
            "%s lateral_y: dist=%+.3fm speed=%.3f duration=%.3fs",
            self.log_prefix,
            dist,
            signed_speed,
            duration,
...
class NavTimed(object):
    ...
    def _decide_obstacle_mode(self):
        ...
        clearances = get_clearances(scan, laser_cfg)
    ...
    def goto_zone(self, zone_name, zone_id=None):
        ...
        rospy.loginfo(
            ...
            "front_sidestep=%s rear_loading=%s",
            label,
            key,
            pose["x"],
            pose["y"],
        ...
        if front_sidestep:
            dist = self._sidestep_distance()
            if zone_name == "loading":
                # 装货区到位后最后一段横移：左移（不再右移回正）
                rospy.loginfo(
                    "[Mission] %s 到位后向左平移 %.2fm", label, dist
                )
                self.nav.go_lateral_y(+dist, speed=spd)
            else:
                rospy.loginfo(
