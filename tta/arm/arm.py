...
class RawSO101Adapter:
    """Direct Feetech register access. Values are raw position ticks."""

    def __init__(self, port: str):
        from start import connect_bus

        self._connect_bus = connect_bus
        self._port = port
        self._bus = None
        self._pose: Dict[str, int] = {}

    def connect(self) -> None:
        self._bus = self._connect_bus(self._port)
    ...
    def read_pose(self) -> Dict[str, float]:
        from start import read_positions

        if self._bus is None:
            raise RuntimeError("Raw motor bus is not connected.")
        return {name: float(value) for name, value in read_positions(self._bus).items()}

    def send(self, pose: Dict[str, float]) -> None:
        from start import enable_torque_safe, move_to

        if self._bus is None:
            raise RuntimeError("Raw motor bus is not connected.")
        ticks = {name: int(round(value)) for name, value in pose.items()}
        invalid = {name: value for name, value in ticks.items() if not RAW_TICK_MIN <= value <= RAW_TICK_MAX}
        if invalid:
            raise ValueError(f"Raw ticks must be in {RAW_TICK_MIN}..{RAW_TICK_MAX}: {invalid}")
        enable_torque_safe(self._bus, list(ticks))
        move_to(self._bus, ticks, wait_s=0.0)
        self._pose = ticks
...
def resolve_raw_poses(config: dict) -> tuple[Dict[str, float], Dict[str, float]]:
    """Load only explicitly labelled raw tick poses; never reinterpret degree poses."""
    grasp = config.get("grasp") if isinstance(config.get("grasp"), dict) else {}
    initial_raw = grasp.get("raw_observe_pose")
    if not isinstance(initial_raw, dict):
        raise ValueError(
            "Raw mode requires grasp.raw_observe_pose in YAML. Run --raw --read, "
            "inspect the pose, then paste its ticks under that key."
        )
    initial = resolve_pose_dict(initial_raw, {name: 2048.0 for name in JOINTS})
    next_raw = grasp.get("raw_next_pose")
    nxt = resolve_pose_dict(next_raw, initial) if isinstance(next_raw, dict) else dict(initial)
    return initial, nxt
...
def print_raw_pose(pose: Dict[str, float]) -> None:
    print("Current raw encoder ticks:")
    for name in JOINTS:
        if name in pose:
            print(f"  {name}: {int(round(pose[name]))}")
    print("YAML raw_observe_pose:")
    print("  raw_observe_pose:")
    for name in JOINTS:
        if name in pose:
            print(f"    {name}: {int(round(pose[name]))}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--raw",
        action="store_true",
        help="Use direct Present_Position/Goal_Position ticks (0..4095), bypassing LeRobot calibration.",
    )
    ...
    if args.read:
        arm = RawSO101Adapter(args.port) if args.raw else SO101Adapter(args.port, args.robot_id)
        try:
    ...
    if args.raw:
        initial_pose, next_pose = resolve_raw_poses(config)
        gripper_open = None
    else:
        initial_pose, next_pose, gripper_open = resolve_poses(config)
    if args.no_open_gripper and not args.raw:
        grasp = config.get("grasp") if isinstance(config.get("grasp"), dict) else {}
